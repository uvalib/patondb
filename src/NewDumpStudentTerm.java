 ...
 * Includes code to add in school/level for Dave G.'s stats
 * Not live yet.
 */
public class NewDumpStudentTerm {
    ...
    static final Logger logger  = LogManager.getLogger(NewDumpStudentTerm.class.getName());

    public NewDumpStudentTerm() {
	initConfiguration("config/sis-pub-db.conf");
	ODSConn = initDbConnection("ods");
	
    }

    private ArrayList<String>  loadPlans() {
	...
	try {
	    ...
	    rs = ps.executeQuery();
	    while (rs.next()) {
		acadPlan = rs.getString("acad_plan");
		plans.add(acadPlan);
		logger.debug("adding - " + acadPlan);
		
	    }
	    ps.close();
	    rs.close();
	} catch (Exception e) {
	    errorExit("Error reading SIS plan input", e);
	} finally {
	    if (ps != null) {
		try {
		    ps.close();
    ...
    private void loadSISData(PrintWriter studentTermWriter, 
			String selectTerm, 
			ArrayList<String> planList)
    {
		...
		"(SELECT  ",
			...
			"term ",
			"FROM ",
			"SYSADM.UV_SR_PROGRAM_TERM ",
			"WHERE ",
        			"term = ?  ",
			"GROUP BY student_system_id, term, term_desc, primary_academic_program, academic_plans )  ",
	...
	try {
	    ...
	    String lastUnivID = "";
	    while (rs.next()) {
		String thisUnivID = rs.getString("university_id");
		/* skip dupe lines for one student */
		/* happens when >1 program active in same term */
		if (thisUnivID.equals(lastUnivID)) {
			continue;
		}
		lastUnivID = thisUnivID;
		String acadPlan = findMajor(rs.getString("academic_plans"),planList);
		fileLine = 
		  thisUnivID + "^" +
		  ...
		  (((holder = rs.getString("primary_academic_program")) == null)? "":holder) + "^" +
		  acadPlan;
    ...
    private String findMajor(String acadPlans, ArrayList<String> planList) {
	if ((acadPlans == null) || acadPlans.equals("")) {
		return "";
	}
	StringTokenizer st = new StringTokenizer(acadPlans,",");
	while (st.hasMoreTokens()) {
		String nextPlan = st.nextToken();	
		if (planList.contains(nextPlan)) {
			return nextPlan;
		}
	}
	return "";
	...
	NewDumpStudentTerm myLoad = new NewDumpStudentTerm();
/*      
 ...
 */
	...
	acadPlans = myLoad.loadPlans();
	myLoad.loadSISData(studentTermWriter, term, acadPlans);
